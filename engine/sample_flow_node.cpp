...
const IdString32 sample_flow_node_name("sample_flow_node");
...
void init_sample_flow_node(GetApiFunction get_engine_api)
{
	...
	_flow_nodes_api->setup_trigger_function(sample_flow_node_name.id(), sample_trigger);
	_flow_nodes_api->setup_set_variable_callback(sample_flow_node_name.id(), sample_set_variable_callback);
	_flow_nodes_api->setup_event_callback(sample_flow_node_name.id(), sample_event_callback);
}

void shutdown_sample_flow_node()
{
	_flow_nodes_api->unregister_flow_node(sample_flow_node_name.id());
