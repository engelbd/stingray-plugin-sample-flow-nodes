...
const IdString32 profiler_flow_node_name("profiler");
...
void init_profiler_flow_node(GetApiFunction get_engine_api)
{
	...
	_flow_nodes_api->setup_trigger_function(profiler_flow_node_name.id(), profiler_trigger);
}

void shutdown_profiler_flow_node()
{
	_flow_nodes_api->unregister_flow_node(profiler_flow_node_name.id());
